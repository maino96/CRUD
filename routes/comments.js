...
router.get("/:_postId", async (req, res) => {
    const { _postIdd } = req.params;
    const detail = await Comments.findOne({ _postId });
...
router.delete("/:_postId", async (req, res) => {
    ...
    if (password === String(originPassword[0].password)) {
      await Comments.deleteOne({ _postId });
  ...
  if (originPassword){
    await Comments.updateOne({ _postId }, { title, content });
  }
  else {   
      return res.json({ message: "댓글을 수정하지 못했습니다." });
    };
